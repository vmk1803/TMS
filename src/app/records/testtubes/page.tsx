...
import TubeTable, { TubeTableRef } from './components/TubeTable';
import Toast from '../../../components/common/Toast';
import { canCreate } from '../../../utils/rbac';

const Page = () => {
  const tableRef = useRef<TubeTableRef>(null);
  ...
  const handleExportRequest = () => {
    if (selectedGuids.length === 0) {
      setToastType('info');
      setToastMessage('Please select tube to export');
      setToastOpen(true);
      return;
    }
  ...
  const handleExportComplete = (count: number) => {
    ...
    setToastMessage(`Successfully exported ${count} tube(s)`);
    setToastOpen(true);
  };
  ...
  return (
    <>
      <div className='flex justify-between'>
        <Title
          heading="Test Tubes"
          subheading="Maintain test tube data with unified search, filtering, and bulk actions."
        />
        <div className="flex gap-3">
          ...
          />
          {canCreate() && (
            <ButtonDarkRounded label="New Test Tube" Icon={PlusIcon} link="/records/testtubes/new" />
          )}
        </div>
      </div>
