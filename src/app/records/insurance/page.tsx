...
import InsuranceTable, { InsuranceTableRef } from './components/InsuranceTable';
import Toast from '../../../components/common/Toast';
import { canCreate } from '../../../utils/rbac';

const Page = () => {
  const tableRef = useRef<InsuranceTableRef>(null);
  ...
  const handleExportRequest = () => {
    if (selectedGuids.length === 0) {
      setToastType('info');
      setToastMessage('Please select insurance to export');
      setToastOpen(true);
      return;
    }
  ...
  const handleExportComplete = (count: number) => {
    ...
    setToastMessage(`Successfully exported ${count} insurance(s)`);
    setToastOpen(true);
  };
  ...
  return (
    <>
      <div className='flex justify-between'>
        <Title
          heading="Insurance"
          subheading="Centralized view of insurance details for easy access and management"
        />
        <div className="flex gap-3">
          ...
          />
          {canCreate() && (
            <ButtonDarkRounded label="New Insurance" Icon={PlusIcon} link="/records/insurance/new" />
          )}
        </div>
      </div>
