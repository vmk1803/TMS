...
import LabTestsTable, { LabTestsTableRef } from './components/LabTestsTable';
import Toast from '../../../components/common/Toast';
import { canCreate } from '../../../utils/rbac';

const Page = () => {
  const tableRef = useRef<LabTestsTableRef>(null);
  ...
  const handleExportRequest = () => {
    if (selectedGuids.length === 0) {
      setToastType('info');
      setToastMessage('Please select lab test to export');
      setToastOpen(true);
      return;
    }
  ...
  const handleExportComplete = (count: number) => {
    ...
    setToastMessage(`Successfully exported ${count} lab test(s)`);
    setToastOpen(true);
  };
  ...
  return (
    <>
      <div className='flex justify-between'>
        <Title
          heading="Lab Tests"
          subheading="Maintain the lab test data with unified search, filtering, and bulk actions."
        />
        <div className="flex gap-3">
          ...
          />
          {canCreate() && (
            <ButtonDarkRounded label="New Lab Tests" Icon={PlusIcon} link="/records/lab-tests/new" />
          )}
        </div>
      </div>
